...
class RemoteSpider(scrapy.Spider):
    ...
    def parse_job(self, response):
        ...
        for job in jobs:
            ...
            job_item["title"] = (
                job.css(".font-weight-bold.larger::text")
                .get()
                .replace("\n", "")
                .replace("&nbsp", ""),
            )
            ...
            job_item["posted_on"] = (
                dateparser.parse(job.css("date::text").get().strip()).strftime(
                    "%Y-%m-%d"
                ),
            )
            job_item["img"] = (
                job.css("div.row.no-gutters.align-items-center img")
                .attrib["data-lazy-src"]
                .strip()
            )
            job_item["company_name"] = (
                job.css(".m-0.text-secondary::text")
                .get()
                .strip()
                .replace("\n", "")
                .replace("  ", "")
            ...
            job_item["skills"] = [
                c.get().replace("&nbsp", "")
                for c in job.css("span.badge.badge-success small::text")
            ]
